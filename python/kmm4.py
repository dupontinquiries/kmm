...
###
class SavingGoal: #### TODO: IMPLEMENT Account.better_faster_project()::isGoalMet(Goal g)
...
if __name__ == "__main__":
    ...
    acc1.better_faster_project(pendulum.today().add(months=3))
    assert acc1.getBalance('USD') == 0
    ...
    acc2.better_faster_project(pendulum.today().add(weeks=2))
    assert acc2.getBalance('USD') == 4500
    assert acc2.getProjectedBalance('USD') == 4500 + 1750.50
    acc2.better_faster_project(pendulum.today().add(weeks=3))
    assert acc2.getProjectedBalance('USD') == 4500 + 1750.50
    acc2.better_faster_project(pendulum.today().add(weeks=4))
    ...
    assert acc1.getBalance('USD') == 0
    assert acc2.getBalance('USD') == 4500
    assert acc1.better_faster_project(pendulum.today().add(days=1)).getBalance('USD') == 0
    assert acc1.better_faster_project(pendulum.today().add(months=1)).getBalance('USD') == 0

    assert acc1.better_faster_project(pendulum.today().add(days=1)).getProjectedBalance('USD') == 600
    assert acc1.better_faster_project(pendulum.today().add(months=1)).getProjectedBalance('USD') == 1200
    # the second and third month are outside the active period, so the payment should only happen today and today+1mo
    assert acc1.better_faster_project(pendulum.today().add(months=3)).getProjectedBalance('USD') == 1200
    print(f"finished testing [{current_test}]\n")
    ...
    assert acc3.better_faster_project(pendulum.today().add(months=3)).getProjectedBalance('USD') == (400.50 + (3 * 900.50))
    print(f"finished testing [{current_test}]\n")
    ...
    for i in range(20):
        ...
        assert epsilon( acc4.better_faster_project(pendulum.today().add(years=5)).getProjectedBalance('USD'), sum(rl) * 12. * 5., .0001)
        for n in rl[:10]:
